// path: /components/giftlist/GiftCard.tsx
"use client";
...
export function GiftCard({
  ...
  const types = {
    "non registry": {
      label: "Non Registry",
      badge: "bg-[#EAFBF3] text-[#2F6D5A] border-[#A8E6CF]",
    },
    monetary: {
      label: "Monetary",
      badge: "bg-[#EAFBF3] text-[#2F6D5A] border-[#A8E6CF]",
  ...
  const dateLabel = formatYmd(gift.date, {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
  ...
  // Reuse one message everywhere to satisfy TS/ESLint and keep copy consistent
  const tooltipMsg = "Available after you create an account";

  return (
    <>
      <Card className="group relative overflow-hidden rounded-2xl border border-gray-100 bg-white transition-shadow shadow-sm hover:shadow-md md:min-h-[320px] w-full py-3 md:py-6">
        <article aria-labelledby={`gift-${gift.id}-title`}>
          <div
            className={`absolute inset-y-0 left-0 w-1 ${
              gift.thankYouSent ? "bg-[#A8E6CF]" : "bg-gray-200"
            }`}
          />
          <CardContent className="flex h-full flex-col p-4 pb-0 md:p-5 relative">
            ...
            >
              <div className="min-w-0">
                <h3
                  ...
                  className="font-bold text-[#1a1a1a] text-2xl leading-snug break-words"
                >
                  {gift.guestName}
                </h3>
            ...
            <div className="mt-3">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {gift.thankYouSent ? (
                  ...
                  <Badge className="border border-[#A8E6CF]/50 text-[#2F6D5A]" variant="outline">
                    Not Thanked
                  </Badge>
                )}
                {noteBadge}
            ...
            <div className="mt-4 hidden md:block">
              <div className="w-full max-w-[520px] mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-3">
                  ...
                  {/* Note Draft — visible for all; disabled in guest mode with tooltip */}
                  <Tooltip open={guestMode ? undefined : false}>
                    <TooltipTrigger asChild>
                      <span className="inline-block w-full">
                        <Button
                          ...
                          size="sm"
                          className="w-full h-10 rounded-xl border border-[#A8E6CF]/50 bg-white text-[#2d2d2d] shadow-sm hover:bg-[#EAFBF3]
                                     focus-visible:ring-2 focus-visible:ring-[#A8E6CF] focus-visible:ring-offset-0 justify-center whitespace-nowrap disabled:opacity-60"
                          onClick={() => {
                            if (!guestMode) onCompose();
                          }}
                          disabled={guestMode}
                    ...
                    {guestMode && <TooltipContent side="top">{tooltipMsg}</TooltipContent>}
                  </Tooltip>
                </div>
              </div>
            ...
            <div className="md:hidden mt-2 flex justify-center">
              ...
              >
                <ChevronDown
                  className={`h-8 w-8 text-[#6B7280] transition-transform ${
